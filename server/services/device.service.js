...
/** Device is Online only if last child status ping within this window (ms). */
...
function setDevice(id, patch) {
    ...
    if (typeof next.lastSeen === 'number' && next.lastSeen > 0) {
        next.isOnline = (Date.now() - next.lastSeen) < ONLINE_WINDOW_MS;
    }
    devices.set(id, next);
    return next;
}
...
function getDeviceStatus(id) {
    ...
    const isOnline = d.lastSeen && (Date.now() - d.lastSeen) < ONLINE_WINDOW_MS;
    return {
        id: id,
        deviceId: id,
        isOnline: !!isOnline,
