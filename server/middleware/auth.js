...
async function requirePairedDevice(req, res, next) {
    try {
        ...
        if (!cred && (url.includes('/status') || url.includes('/upload_data') || url.includes('/upload-data') || url.includes('/upload-image') || url.includes('/upload_image') || url.includes('/upload-audio') || url.includes('/upload-storage-file') || url.includes('/upload-storage-chunk'))) {
            req.deviceId = raw;
            req.deviceCredential = null;
