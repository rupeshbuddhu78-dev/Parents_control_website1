...
async function getAuditLogs(req, res) {
    try {
        ...
        res.json({ logs, total: result.total || logs.length });
    } catch (e) {
...
async function uploadVideo(req, res) {
    try {
        ...
        // Save to database (title = phone brand for setup, or label for control)
        const title = (req.body.title || req.body.brand || '').toString().trim();
