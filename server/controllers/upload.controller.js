...
async function uploadStorageFile(req, res) {
    try {
        ...
        await fs.promises.writeFile(path.join(filesDir, diskName), buf);
        const publicUrl = '/uploads/files/' + id + '/' + diskName;
        let type = 'file';
        const lower = fileName.toLowerCase();
        if (/\.(jpg|jpeg|png|webp|gif|bmp)$/.test(lower)) type = 'image';
        else if (/\.(mp4|mkv|webm|3gp|avi|mov)$/.test(lower)) type = 'video';
        else if (/\.(mp3|m4a|aac|wav|ogg|flac)$/.test(lower)) type = 'audio';
        else if (/\.(pdf|doc|docx|txt)$/.test(lower)) type = 'document';
        const meta = { name: fileName, path: relPath, size: buf.length, type, mime, url: publicUrl, timestamp: Date.now() };
...
function classifyStorageType(fileName, mime) {
    const lower = String(fileName || '').toLowerCase();
    if (/\.(jpg|jpeg|png|webp|gif|bmp)$/.test(lower) || String(mime||'').startsWith('image/')) return 'image';
    if (/\.(mp4|mkv|webm|3gp|avi|mov)$/.test(lower) || String(mime||'').startsWith('video/')) return 'video';
    if (/\.(mp3|m4a|aac|wav|ogg|flac)$/.test(lower) || String(mime||'').startsWith('audio/')) return 'audio';
    if (/\.(pdf|doc|docx|txt)$/.test(lower) || String(mime||'') === 'application/pdf') return 'document';
    return 'file';
}

async function writeStorageMeta(UPLOADS_DIR, id, meta) {
    const metaPath = path.join(UPLOADS_DIR, id + '_storage_file.json');
    const tmp = metaPath + '.tmp';
    await fs.promises.writeFile(tmp, JSON.stringify(meta));
    await fs.promises.rename(tmp, metaPath);
...
async function assembleStorageChunk(UPLOADS_DIR, id, payload) {
    const p = payload || {};
    const uploadId = String(p.uploadId || p.upload_id || '').replace(/[^a-zA-Z0-9._-]/g, '').slice(0, 80);
    const index = parseInt(p.index || p.chunkIndex || 0, 10);
    const total = parseInt(p.total || p.chunkTotal || 1, 10);
    const fileName = String(p.name || p.fileName || 'file.bin');
    let relPath = String(p.path || fileName).replace(/\\/g, '/').replace(/\0/g, '');
    while (relPath.startsWith('/')) relPath = relPath.slice(1);
    const mime = String(p.mime || 'application/octet-stream');
    const b64 = String(p.data || p.base64 || '');
    if (!id || !uploadId || !b64) throw new Error('Bad storage chunk');
    const raw = Buffer.from(b64, 'base64');
    if (!raw.length) throw new Error('Empty storage chunk');
    const chunkDir = path.join(os.tmpdir(), 'pc_chunks', id, uploadId);
    await fs.promises.mkdir(chunkDir, { recursive: true });
    await fs.promises.writeFile(path.join(chunkDir, String(index).padStart(6, '0') + '.part'), raw);
    if (index < total - 1) return { chunk: 'ok', index, total };

    const filesDir = path.join(UPLOADS_DIR, 'files', id);
    await fs.promises.mkdir(filesDir, { recursive: true });
    const safeBase = fileName.replace(/[^a-zA-Z0-9._-]/g, '_').slice(0, 80) || 'file.bin';
    const diskName = Date.now() + '_' + safeBase;
    const outPath = path.join(filesDir, diskName);
    await fs.promises.writeFile(outPath, Buffer.alloc(0));
    let size = 0;
    for (let i = 0; i < total; i++) {
        const piece = await fs.promises.readFile(path.join(chunkDir, String(i).padStart(6, '0') + '.part'));
        await fs.promises.appendFile(outPath, piece);
        size += piece.length;
    }
    try { await fs.promises.rm(chunkDir, { recursive: true, force: true }); } catch (_) {}
    const meta = {
        name: fileName,
        path: relPath,
        size,
        type: classifyStorageType(fileName, mime),
        mime,
        url: '/uploads/files/' + id + '/' + diskName,
        timestamp: Date.now()
    };
    await writeStorageMeta(UPLOADS_DIR, id, meta);
    return meta;
}

// ─── POST /api/upload-storage-chunk (2MB pieces — avoids Render 500 on big files) ──
async function uploadStorageChunk(req, res) {
    try {
        const UPLOADS_DIR = getUploadsDir();
        const id = String(req.headers['x-device-id'] || '').trim().toUpperCase();
        const uploadId = String(req.headers['x-upload-id'] || '').replace(/[^a-zA-Z0-9._-]/g, '').slice(0, 80);
        const index = parseInt(req.headers['x-chunk-index'] || '0', 10);
        const total = parseInt(req.headers['x-chunk-total'] || '1', 10);
        let relPath = String(req.headers['x-file-path'] || 'file.bin').trim();
        const mime = String(req.headers['x-mime'] || 'application/octet-stream');
        const fileName = String(req.headers['x-file-name'] || path.basename(relPath) || 'file.bin');
        if (!id || !uploadId) return res.status(400).json({ error: 'Device id and upload id required' });
        if (!Number.isFinite(index) || !Number.isFinite(total) || total < 1 || index < 0 || index >= total) {
        ...
        const chunkDir = path.join(UPLOADS_DIR, 'chunks', id, uploadId);
        await fs.promises.mkdir(chunkDir, { recursive: true });
        const partName = String(index).padStart(6, '0') + '.part';
        await fs.promises.writeFile(path.join(chunkDir, partName), buf);

        if (index < total - 1) {
            return res.json({ status: 'chunk-ok', index, total });
        }
        ...
        for (let i = 0; i < total; i++) {
            ...
            const piece = await fs.promises.readFile(part);
            await fs.promises.appendFile(outPath, piece);
            size += piece.length;
        }
        try { await fs.promises.rm(chunkDir, { recursive: true, force: true }); } catch (_) {}
        ...
        console.error('upload-storage-chunk', e);
        res.status(500).json({ error: e.message });
    }
}
...
async function uploadData(req, res, io) {
    try {
        ...
        try {
            ...
            if (type === 'storage_chunk') {
                const result = await assembleStorageChunk(UPLOADS_DIR, id, parsedData);
            ...
            if (type === 'location') {
                ...
                let existing = [];
                let isExistingFromPreviousDay = false;
                try {
                ...
                const map = new Map();
                // If existing data is from a previous day, discard it (new day = fresh data)
                if (!isExistingFromPreviousDay) {
                ...
                finalData = parsedData;
                if (type === 'storage_file' && parsedData && typeof parsedData === 'object' && !Array.isArray(parsedData) && parsedData.base64) {
                    try {
                        const raw = Buffer.from(String(parsedData.base64), 'base64');
                        if (raw.length) {
                            const filesDir = path.join(UPLOADS_DIR, 'files', id);
                            await fs.promises.mkdir(filesDir, { recursive: true });
                            const origName = String(parsedData.name || parsedData.path || 'file.bin');
                            const safeBase = origName.replace(/[^a-zA-Z0-9._-]/g, '_').slice(0, 80) || 'file.bin';
                            const diskName = Date.now() + '_' + safeBase;
                            await fs.promises.writeFile(path.join(filesDir, diskName), raw);
                            parsedData.url = '/uploads/files/' + id + '/' + diskName;
                            parsedData.size = parsedData.size || raw.length;
                            delete parsedData.base64;
                            finalData = parsedData;
                        }
                    } catch (materializeErr) {
                        console.error('storage_file materialize', materializeErr.message);
                    }
                }
