...
async function getSubscriptionStatus(req, res) {
    try {
        ...
        if (!user || !user.activeSubscription) {
            return res.json({ hasActiveSubscription: false, subscription: null, freeAll: false, featureLocks: featureLocks || {} });
        }
        ...
        if (!sub || sub.status !== 'active' || sub.expiryDate < new Date()) {
            ...
            return res.json({ hasActiveSubscription: false, subscription: sub, freeAll: false, featureLocks: featureLocks || {} });
        }
        const planPrice = Number(sub.planId && sub.planId.price);
        const isPaidPlan = Number.isFinite(planPrice) && planPrice > 0;
        res.json({ hasActiveSubscription: isPaidPlan, subscription: sub, freeAll: false, featureLocks: featureLocks || {} });
    } catch (e) {
...
function parsePlanValues(body, { requireName = false } = {}) {
    ...
    // Gallery & Cloudinary limits
    const maxPhotos = body.maxPhotos !== undefined ? Math.max(0, Number(body.maxPhotos) || 0) : undefined;
    const cloudinaryPhotoLimit = body.cloudinaryPhotoLimit !== undefined ? Math.max(0, Number(body.cloudinaryPhotoLimit) || 0) : undefined;
    const cloudinaryVideoLimit = body.cloudinaryVideoLimit !== undefined ? Math.max(0, Number(body.cloudinaryVideoLimit) || 0) : undefined;
    ...
    return {
        ...
        ...(cloudinaryPhotoLimit !== undefined ? { cloudinaryPhotoLimit } : {}),
        ...(cloudinaryVideoLimit !== undefined ? { cloudinaryVideoLimit } : {}),
...
async function adminUpdatePlan(req, res) {
    try {
        ...
        // If name is being changed, also regenerate slug
        if (req.body.name && req.body.name !== existing.name) {
            merged.slug = normalizePlanSlug(req.body.slug || req.body.name);
        }
        const values = parsePlanValues(merged, { requireName: true });
...
async function adminDeletePlan(req, res) {
    try {
        const plan = await Plan.findById(req.params.id);
        if (!plan) return res.status(404).json({ error: 'Plan not found' });
        ...
        if (activeSubs > 0) {
            return res.status(400).json({ error: `Cannot delete: ${activeSubs} active subscription(s) are using this plan. Disable the plan instead.` });
        }
...
module.exports = {
    ...
    adminGetPlans, adminCreatePlan, adminUpdatePlan, adminTogglePlan, adminDeletePlan,
};
