...
async function start() {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('join', (room) => {
            ...
            if (base) {
                socket.join(base);
                if (roomId !== base) socket.join(roomId);
            } else {
                socket.join(roomId);
            }
            console.log('[Socket]', socket.id, 'join', roomId, 'base=', base || '');
        });
        ...
        socket.on('send-command', async (payload) => {
            try {
                ...
                const cmdObj = (typeof cmdPayload === 'object' && cmdPayload)
                    ? { ...cmdPayload, command, targetId: deviceId, deviceId: deviceId }
                    : { command: String(command), targetId: deviceId, deviceId: deviceId };
                io.to(deviceId).emit('command', cmdObj);
                // Mic: also emit control-event ONLY to that device room (never broadcast)
                const c = String(command || '').toLowerCase();
                if (c === 'start-mic' || c === 'start_audio' || c === 'stop-mic' || c === 'stop_audio') {
                    const action = (c.indexOf('start') >= 0) ? 'start-mic' : 'stop-mic';
                    io.to(deviceId).emit('control-event', {
                        action, room: deviceId, deviceId, targetId: deviceId, target: deviceId
                    });
                }
        ...
        const relayEvents = [
            ...
            'storage-file-request', 'storage-offer', 'storage-answer', 'storage-candidate',
            'storage-error', 'storage-file-close',
            ...
            'control-event',
            'switch-camera',
            'audio-offer', 'audio-answer', 'audio-candidate', 'audio-p2p-request',
        ];
        ...
        for (const event of relayEvents) {
            socket.on(event, (data) => {
                ...
                if (event === 'gallery-request' || event === 'gallery-fallback-request' || event === 'gallery-relay-start' || event === 'storage-file-request') {
                ...
                // Include payload.room — parent control-event / mic uses { room: DEVICE_ID, action }
                const room = payload.target || payload.deviceId || payload.device_id
                    || payload.targetRoom || payload.targetId || payload.room;
                if (room) {
                    const roomStr = String(room);
                    const base = normalizeDeviceId(roomStr);
                    ...
                    if (base) {
                        ...
                        socket.to(base + '_audio').emit(event, payload);
                    }
                    console.log(`[Socket] relay ${event} -> room ${roomStr}` + (base && base !== roomStr ? ` (+${base})` : ''));
                    return;
                }

                // NEVER broadcast control-event / mic commands to all devices
                if (event === 'control-event') {
                    console.warn('[Socket] control-event dropped: missing room/targetId', payload && payload.action);
                    return;
                }
